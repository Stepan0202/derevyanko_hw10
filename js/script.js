...
const hw10_answer = document.querySelector('.hw10_answ0');
let hw10_phonesAndBills = {};
let hw10_balanceSum = 0;
hw10_input.forEach(user => {
    let balance = parseFloat(user.balance.replace(/[^\d.]/g, ''));
    hw10_balanceSum += balance;
    if(balance > 2000){
        hw10_phonesAndBills[user.phone] = balance; 
    }
});

let hw10_usersNumbers = Object.keys(hw10_phonesAndBills);
hw10_answer.innerHTML = `Array of the phones: ${hw10_usersNumbers}. </br> Sum of the balances of all users: ${hw10_balanceSum.toFixed(2)}`;
